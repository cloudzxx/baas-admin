...
#   - restart:        Stop the baas-admin service and then start
...
#   - start:          Start the baas-admin service
#   - stop:           Stop the baas-admin service, and remove all service containers
...
BASENAME ?= $(DOCKER_NS)/baas-admin
AGENT_BASENAME ?= $(DOCKER_NS)/baas-admin-agent
VERSION ?= 0.9.0
IS_RELEASE=false
...
# The FXBaaS Admin service listen interface, please use the public available IP.
SERVER_PUBLIC_IP ?= 127.0.0.1

LOCAL_STORAGE_PATH=/opt/baas-admin

# Docker images needed to run baas-admin services
...
clean: ##@Clean Stop services and clean docker containers.
	make stop
	@-docker ps -a --filter "name=baas-admin-" --format "{{.ID}}" | xargs docker rm -f >/dev/null 2>&1
...
## Help rules
clean-images: 
	@echo "Clean all baas-admin related images, may need to remove all containers"
	make clean
	@-docker images --filter=reference='hyperledger/*baas-admin*' --format '{{.ID}}' | xargs -r docker rmi -f 2>/dev/null  
	@echo "Images deleted!"
...
fisco-agent:
	docker build -t baas-admin/fisco-agent:latest src/agents/fisco-bcos
