...
from hyperledger_fabric.settings import FXBAAS_HOME
...
class ChaincodeCreationSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        chaincode_dir = os.path.join(FXBAAS_HOME, channel_name, "chaincodes")
...
class ChaincodeInstallationSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        chaincode_dir = os.path.join(FXBAAS_HOME, validated_data["channel_name"], "chaincodes")
        os.makedirs(chaincode_dir, exist_ok=True)
        fs = FileSystemStorage(location=chaincode_dir)
        filename = fs.save(
            "{}_{}_{}.tar.gz".format(
