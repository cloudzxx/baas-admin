...
from api_engine.settings import FXBAAS_HOME, FABRIC_PEER_CFG, FABRIC_ORDERER_CFG, FABRIC_VERSION
...
def create(organization: Organization, node_type: Node.Type, node_name: str, blockchain_type: str = "fabric") -> Node:
    driver = get_driver(blockchain_type)
    result = driver.create_node(name=node_name, node_type=str(node_type), organization=organization)
    node = Node(
        name=node_name,
        type=node_type,
...
def _generate_peer_config(organization_name: str, peer_domain_name: str) -> None:
    _generate_config(
        ...
        **{
            ...
            "vm_docker_hostConfig_NetworkMode": "baas_admin_net",
...
def _get_msp(organization_name: str, node_type: Node.Type, node_domain_name: str) -> bytes:
    directory_path = get_node_directory(organization_name, node_type, node_domain_name)
...
def get_peer_directory(organization_name: str, peer_domain_name: str):
    return get_node_directory(organization_name, Node.Type.PEER, peer_domain_name)


def get_orderer_directory(organization_name: str, orderer_domain_name: str):
    return get_node_directory(organization_name, Node.Type.ORDERER, orderer_domain_name)


def get_node_directory(organization_name: str, node_type: Node.Type, node_domain_name: str) -> str:
    return "{}/{}s/{}".format(
...
def get_node_env(node_type: Node.Type, node_domain_name: str, msp, tls, cfg) -> Optional[Dict[str, Any]]:
    if node_type == Node.Type.PEER:
...
def _get_peer_env(peer_domain_name: str, msp, tls, cfg) -> Dict[str, Any]:
    return {
        ...
        "CORE_VM_DOCKER_HOSTCONFIG_NETWORKMODE": "baas-admin-net",
...
def get_node_cmd(node_type: Node.Type) -> Optional[str]:
    if node_type == Node.Type.PEER:
